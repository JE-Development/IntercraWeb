...
export class ITunesMovies implements PluginInterface, FeedInterface{
    finish = false;
    contentList: Map<string, string>[] = [];
    page: number = 1;

    displayName = "iTunes Movies";
    id = "itunes_movies";
    ...
    async startSearch(searchText: string, pc: PluginController): Promise<void>{
        ...
        await hrc.httpRequest(
            "https://itunes.apple.com/search?term=" + searchText + "&limit=50&media=movie",
            pc, this.id).then(r =>
            this.analyse(r)
        );
    }
